...
class Logger
{
     ...
     *   @type array  $transport    Required. { type: 'file'|'http', config?: array }
    ...
    private function _createTransporter(array $config)
    {
        ...
        if (!is_dir($basePath) && !mkdir($basePath, 0777, true) && !is_dir($basePath)) {
            throw new \RuntimeException(
                '[uengage-logger] Cannot create log directory "' . $basePath . '"'
            );
        }

        $transportConfig['filePath'] = rtrim($basePath, '/\\')
                                       . DIRECTORY_SEPARATOR
                                       . $config['product'] . '-' . $config['service'] . '.log';
    ...
    private function _validateConfig(array $config)
    {
        ...
        if (!isset($config['transport']['type'])
            || !in_array($config['transport']['type'], array('http', 'file'), true)) {
            throw new \InvalidArgumentException("config.transport.type must be 'http' or 'file'");
        }
    }
