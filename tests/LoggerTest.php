...
class LoggerTest extends \Yoast\PHPUnitPolyfills\TestCases\TestCase
{
    ...
    protected function set_up()
    {
        ...
        $this->_logFile = $this->_tmpDir . DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR . 'edge.log';
    }
    ...
    public function test_createsLogFileAtBasePathApplicationProductDotLog()
    {
    ...
    public function test_autoCreatesDirectoryIfNotExists()
    {
        ...
        $this->assertFileExists($nestedDir . DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR . 'edge.log');
    }

    // ─── Group 3b: Stdout transport ───────────────────────────────────────────

    public function test_stdoutTransportConstructsWithoutError()
    {
        // Stdout transport has no configuration; it just opens php://stdout.
        // The actual emission goes to the PHP process's stdout stream and
        // can't be intercepted via output buffering, so we only verify
        // construction + that send() doesn't throw.
        $logger = new Logger(array_merge($this->_fileConfig(), array(
            'transport' => array('type' => 'stdout'),
        )));
        $this->assertInstanceOf('Uengage\Logger\Logger', $logger);
        // Should not throw. Output (if any during a unit test) is harmless.
        $logger->warn('stdout-smoke-test');
    }
