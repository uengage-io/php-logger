...
| -------------- | ------------------------------------------------------------ | -------------------------------------------------------------------------------- |
| `file`         | Appends NDJSON lines to `{basePath}/{product}-{service}.log` | EC2/server services — CloudWatch Agent, Datadog Agent, or Fluentd ships the file |
...
$logger = new Logger(array(
    ...
    // log file created at: /var/log/uengage/edge-ordering.log
));
...
Appends one JSON object per line (NDJSON format) to a log file. The **filename is derived automatically** from your `product` and `service` config — you only provide the base directory.
...
Logger
  ...
  └── _transporter->send($entry)
        ├── FileTransporter
        │     ├── path: {basePath}/{product}-{service}.log   (derived at init, dir auto-created)
